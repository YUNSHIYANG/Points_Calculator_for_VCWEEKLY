...
def read_base_from_excel(file_path, sheet_name):
    """从第11行开始读取历史数据并累加，返回历史累计基数（字典）"""
    try:
        wb = openpyxl.load_workbook(file_path, data_only=True)
        ws = wb[sheet_name]
        
        total = {'view': 0, 'like': 0, 'danmaku': 0, 'reply': 0, 'coin': 0, 'favorite': 0}
        ...
        row = 11
        while True:
            view_val = ws.cell(row, 2).value  # B列
            # 空行判断：None 或 空字符串
            if view_val is None or (isinstance(view_val, str) and view_val.strip() == ''):
                break
            total['view'] += safe_float(view_val)
            total['like'] += safe_float(ws.cell(row, 3).value)
            total['danmaku'] += safe_float(ws.cell(row, 4).value)
            total['reply'] += safe_float(ws.cell(row, 5).value)
            total['coin'] += safe_float(ws.cell(row, 6).value)
            total['favorite'] += safe_float(ws.cell(row, 7).value)
            row += 1
        
        wb.close()
        return total
    except Exception as e:
        messagebox.showerror("读取基数失败", f"无法读取历史数据：{e}")
        return None
